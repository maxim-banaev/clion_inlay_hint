...
template <typename T> void test_template_parameter_type(T t) {
  auto at = t; // T
}
...
void test_std_type() {
  ...
  const auto tuple =
      std::make_tuple(1, 'a', 2.3); // const tuple<int, char, double>
  const auto tuple1 = std::make_tuple(1, std::string("b"),
                                      2.3); // const tuple<int, string, double>

  auto aList = {1, 2, 3, 4}; // initializer_list<int>
}

void test_pair(const std::unordered_map<std::string, int> &v,
               const std::multimap<std::string, int> &s) {
...
template <class T> void test_iterator(typename T::iterator Iter) {
